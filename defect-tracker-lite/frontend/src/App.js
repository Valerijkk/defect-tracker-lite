...
const STATUS_LABEL = {
  all:'Все', new:'New', in_progress:'In progress', review:'Review', closed:'Closed', cancelled:'Cancelled'
};
const STATUS_COLOR = {
  new:'info', in_progress:'warning', review:'secondary', closed:'success', cancelled:'default'
};
...
function Dashboard() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [p, d] = await Promise.all([api.get('/projects', auth), api.get('/defects', auth)]);
  ...
  const filtered = useMemo(() => {
    const st = STATUS[tab];
    return defects.filter(d => (projectId ? d.project_id === Number(projectId) : true) && (st==='all' ? true : d.status===st));
  }, [defects, tab, projectId]);
  ...
  const createDefect = async () => {
    try {
      ...
      await api.post('/defects', newDefect, auth);
  ...
  const createProject = async () => {
    try {
      ...
      await api.post('/projects', newProject, auth);
